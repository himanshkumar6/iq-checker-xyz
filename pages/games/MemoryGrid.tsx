    ...
    <div className="min-h-screen overflow-hidden bg-transparent">
      ...
      <div className="pt-16 pb-8 bg-slate-900/20 light:bg-slate-50/10">
        <div className="container mx-auto px-4">
          ...
          <div className="flex justify-between items-start mb-8">
            <div>
              <h1 className="text-3xl md:text-4xl font-extrabold text-slate-50 light:text-slate-900 mb-2">Memory Grid Challenge</h1>
              <p className="text-slate-300 light:text-slate-600">Retain and recall structural patterns</p>
            </div>
          ...
          <div className="grid grid-cols-4 gap-3 mb-8">
            ...
            <div className="p-3 glass bg-slate-900 light:bg-white/70 backdrop-blur-md border border-slate-800 light:border-slate-200 rounded-xl text-center shadow-sm">
              <p className="text-xs text-slate-500 light:text-slate-400 uppercase mb-1">Completed</p>
            ...
            <div className="p-3 glass bg-slate-900 light:bg-white/70 backdrop-blur-md border border-slate-800 light:border-slate-200 rounded-xl text-center shadow-sm">
              ...
              <p className="text-lg font-bold text-slate-50 light:text-slate-900">{level}</p>
            </div>
            <div className="p-3 glass bg-slate-900 light:bg-white/70 backdrop-blur-md border border-slate-800 light:border-slate-200 rounded-xl text-center shadow-sm">
              <p className="text-xs text-slate-500 light:text-slate-400 uppercase mb-1">Grid Area</p>
              <p className="text-lg font-bold text-slate-50 light:text-slate-900">{gridSize}×{gridSize}</p>
            </div>
          </div>

          {/* Phase Indicator */}
          <div className="flex items-center justify-center gap-3 mb-8">
            <div className={`flex items-center gap-2 px-4 py-2 rounded-full ${phase === 'memorize' ? 'bg-blue-600/20 text-blue-400' : 'bg-slate-950 light:bg-slate-100 text-slate-500'
              }`}>
              <Eye className="w-4 h-4" />
              <span className="text-sm font-bold">Memorize</span>
            </div>
            <div className={`flex items-center gap-2 px-4 py-2 rounded-full ${phase === 'recall' ? 'bg-emerald-600/20 text-emerald-400' : 'bg-slate-950 light:bg-slate-100 text-slate-500'
              }`}>
      ...
      <div className="px-4 pb-16 bg-transparent pt-16">
          ...
          <div className="max-w-lg mx-auto">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="p-8 glass bg-slate-900 light:bg-white/70 backdrop-blur-md rounded-3xl border border-slate-800 light:border-slate-200 shadow-sm"
              >
                {/* Instruction */}
                <div className="text-center mb-6">
                  {phase === 'memorize' && (
                    <p className="text-slate-400 text-sm">
                      <Eye className="w-4 h-4 inline mr-2 text-blue-400" />
                ...
                >
                  {Array.from({ length: totalCells }).map((_, index) => {
                    ...
                    let bgClass = 'bg-slate-950 light:bg-slate-100';
                    ...
                    return (
                      <motion.button
                        ...
                        className={`aspect-square ${bgClass} rounded-xl border-2 border-slate-800 light:border-slate-200 transition-all hover:scale-105 active:scale-95 disabled:cursor-default shadow-sm`}
                ...
                {phase === 'recall' && (
                  <div className="mt-6">
                    <div className="flex justify-between text-xs text-slate-500 light:text-slate-400 mb-2">
                      <span>Progress</span>
                      <span>{userPattern.length} / {pattern.length}</span>
                    </div>
                    <div className="w-full h-2 bg-slate-900 light:bg-slate-100 rounded-full overflow-hidden border border-slate-800 light:border-slate-200">
                      <motion.div
                        className="h-full bg-emerald-600 light:bg-emerald-500"
                        initial={{ width: 0 }}
        ...
        <QuitConfirmationModal
          ...
          gameName="Memory Grid Challenge"
        />
      </div>
    </div>
  );
